...
def evaluate(gold_list, predicted_list):
    ...
    arg1_cm, arg2_cm, rel_arg_cm = \
        evaluate_argument_extractor(gold_list, predicted_list)
    sense_cm = evaluate_sense(gold_list, predicted_list)

    print 'Explicit connectives         :' \
        ' Precision %1.4f Recall %1.4f F1 %1.4f' % connective_cm.get_prf('yes')
    print 'Arg 1 extractor              :' \
        ' Precision %1.4f Recall %1.4f F1 %1.4f' % arg1_cm.get_prf('yes')
    print 'Arg 2 extractor              :' \
        ' Precision %1.4f Recall %1.4f F1 %1.4f' % arg2_cm.get_prf('yes')
    print 'Arg1 Arg2 extractor combined :' \
        ' Precision %1.4f Recall %1.4f F1 %1.4f' % rel_arg_cm.get_prf('yes')
    ...
    return (connective_cm, arg1_cm, arg2_cm, rel_arg_cm, sense_cm,
            precision, recall, f1)
...
def evaluate_connectives(gold_list, predicted_list):
    """Evaluate connective recognition accuracy for explicit relations
    ...
    explicit_predicted_list = [(x['DocID'], x['Connective']['TokenList'])
                               for x in predicted_list if
                               x['Type'] == 'Explicit']
    connective_cm = compute_binary_eval_metric(explicit_gold_list,
                                               explicit_predicted_list,
...
def spans_exact_matching(gold_doc_id_spans, predicted_doc_id_spans):
    ...
    for gold_span, predicted_span in zip(gold_spans, predicted_spans):
        exact_match = span_exact_matching((gold_docID, gold_span),
...
def span_exact_matching(gold_span, predicted_span):
    ...
    return gold_docID == predicted_docID and \
        gold_token_indices == predicted_token_indices
...
def evaluate_sense(gold_list, predicted_list):
    ...
    The label ConfusionMatrix.NEGATIVE_CLASS is for the relations that are
    missed by the system because the arguments don't match any of the gold
    relations.
    ...
    for i, gold_relation in enumerate(gold_list):
        if i in gold_to_predicted_map:
            ...
            sense_cm.add(ConfusionMatrix.NEGATIVE_CLASS,
                         gold_relation['Sense'][0])
...
def main():
    ...
    explicit_predicted_list = [x for x in predicted_list if
                               x['Type'] == 'Explicit']
