...
def write_results(prefix, result_tuple, output_file):
    connective_cm, arg1_cm, arg2_cm, rel_arg_cm, \
        sense_cm, precision, recall, f1 = result_tuple
    ...
    write_proto_text('%s Explicit connective precision' %
                     prefix, p, output_file)
    ...
    write_proto_text('%s Arg 1 Arg2 extraction precision' %
                     prefix, p, output_file)
    write_proto_text('%s Arg 1 Arg2 extraction recall' %
                     prefix, r, output_file)
...
def write_partial_match_results(prefix, result_tuple, output_file):
    arg1_match_prf, arg2_match_prf, entire_relation_match_prf, \
        parser_prf = result_tuple
    ...
    write_proto_text('%s Arg1 extraction precision' %
                     prefix, precision, output_file)
    ...
    write_proto_text('%s Arg2 extraction precision' %
                     prefix, precision, output_file)
...
def main(args):
    ...
    gold_relations = [json.loads(x) for x in open('%s/relations.json' %
                                                  input_dataset)]
    predicted_relations = [json.loads(x) for x in open('%s/output.json' %
                                                       input_run)]
    ...
    write_results('Explicit only',
                  evaluate(explicit_gold_relations,
                           explicit_predicted_relations),
                  output_file)
    ...
    write_results('Non-explicit only',
                  evaluate(non_explicit_gold_relations,
                           non_explicit_predicted_relations), output_file)

    write_partial_match_results('All (partial match)',
                                partial_evaluate(gold_relations,
                                                 predicted_relations, 0.7),
                                output_file)
    write_partial_match_results('Explicit only (partial match)',
                                partial_evaluate(explicit_gold_relations,
                                                 explicit_predicted_relations,
                                                 0.7),
                                output_file)
    write_partial_match_results(
        'Non-explicit only (partial match)',
        partial_evaluate(non_explicit_gold_relations,
                         non_explicit_predicted_relations, 0.7),
        output_file)
